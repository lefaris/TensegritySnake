...
M = readmatrix('OPP TENDON FAILED UNDULATION.xlsx','Range','C6:BD11307');
final_row = 11302;
...
% Generate video info
video_title = "undulation_tendon_fail_tracking.avi"; %Update with descriptive title and make sure not to overwrite previous video
...
% Manually set time that different gaits occur in a movement sequence
figure(3)
for i = [1, 11302] % Plot starting and ending full robot configurations
    % For module 1 since we have all points, we have to end at the starting node (1)
    EndpointsMod1 = [E1(i,:); E2(i,:); E3(i,:); E1(i,:)];
    fill3(EndpointsMod1(:,1),EndpointsMod1(:,2),EndpointsMod1(:,3), 'g-o')
    hold on
...
%view(0,0)
title(t, 'Undulation Gait Tendon Fail Tracked');
